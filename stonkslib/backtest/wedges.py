...
def backtest_wedges(ticker, interval, strategy=None):
    ...
    if strategy:
        threshold = (
            strategy.get("patterns", {})
            .get("wedges", {})
            .get("params", {})
            .get("confidence_threshold", DEFAULT_CONFIDENCE_THRESHOLD)
        )
...
def run_all_backtests(strategy=None):
    ...
    for ticker in tickers:
        for interval in intervals:
            backtest_wedges(ticker, interval, strategy=strategy)
